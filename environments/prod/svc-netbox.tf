################################################################################
# BY-SYSTEMS — NetBox (IPAM / Source of Truth)
# Node: srv-proxmox-poc-01 | env=prod | SVC zone (vlan1030, 10.1.3.0/24)
#
# Nested LXC running NetBox (web + worker + housekeeping) as Docker containers (ansible:
...
module "svc_netbox" {
  ...
  env         = "prod"
  os_type     = "debian"
  tags        = ["service", "vlan1030", "zone-svc", "role-ipam", "netbox", "docker"]
  ...
  vlan_tag       = 1030
  ipv4_address   = "10.1.3.120/24"
  ipv4_gateway   = "10.1.3.1"
  ipv6_address   = "fd01:3::120/64"
...
output "svc_netbox" {
  ...
  value = {
    ...
    ipv6 = "fd01:3::120"
    vlan = 1030
    fqdn = "lxc-nbox-01.by-research.be"
  }
}
